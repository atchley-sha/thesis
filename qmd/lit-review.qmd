...
These models use an activity or tour scheduler to assign a \acr{DAP} of zero or more tours to each individual, where a tour is a series of trips that begin and end at home.
...
For example, it could predict that the individual takes transit to work but then drives home or that the individual makes two trips to recreation without ever making a return trip.
...
It is often taken as a given that \acr{ABM}s are unilaterally superior to traditional trip-based models due to their better theoretical foundation, but it is not clear if that better foundation always yields better results in terms of analytical flexibility or policy outcomes.
