# Literature Review {#lit-review-lit-review-lit-review-lit-review-lit-review-lit-review-sec-literature}
...
\acr{ABM}s therefore attempt to present a more conceptually accurate model of actual travel behavior than traditional trip-based models.

Despite these advantages, many agencies have yet to adopt \acr{ABM}s, and instead continue to use trip-based models [@miller_current_2023].
...
It is also not always clear if \acr{ABM}s provide substantially better forecasts than their trip-based counterparts, nor if this tradeoff is worth the increased costs for every agency.
...
In a typical trip-based model, travel demand is predicted based on aggregate population data, often delineated by \acr{TAZ}.
...
This becomes a particular issue in equity analysis because it is perhaps impossible to determine equitable distribution of "winners" and "losers" of a potential policy without using demographic variables in the trip generation and destination and mode choice steps [@bills_looking_2017].
Though many studies have shown that trip production and mode choice behavior differ by ethnic group even after controlling for income [@yum_association_2020; @zmud_invluence_2001; @bhat_travel-related_2013], including such variables in trip-based models is problematic.
...
Regardless of the reasons for their exclusion, these variables consequently cannot be used in a post-hoc analysis of a transportation policy because the trip matrices do not contain the adequate segmentation.
...
In the aggregate data model, it is impossible to know which trips are made by e.g. 2-worker, 1-vehicle, low-income households after the mode choice step; it only describes which trips are made by households with fewer vehicles than workers.
...
::: {#lit-review-lit-review-lit-review-fig-pipeline-example layout-ncol="1"}
![Aggregate data](../images/aggregate.png){#lit-review-lit-review-lit-review-fig-pipeline-example-1}

![Synthetic population](../images/synthetic.png){#lit-review-lit-review-lit-review-fig-pipeline-example-2}
...
The hope, though, is that over an entire population the inconsistencies would cancel out, leaving an overall accurate forecast.
...
Figures [-@fig-network-aggregate] and [-@fig-network-synth] show an example of the trips distributed across several TAZs in the various model forms.
...
![Example trip distribution using aggregate data. There is little information on who is making which trips, and it is not known how trips are related to each other.](../images/tbm.png){#lit-review-lit-review-lit-review-lit-review-lit-review-lit-review-fig-network-aggregate fig-scap="Example network assignment using aggregate data."}

````{=html}
<!--

```{r}
#| label: fig-network-syntha
...
# knitr::include_graphics("./images/trip.png")
# knitr::include_graphics("./images/tour.png")
```

-->
````

::: {#lit-review-lit-review-lit-review-lit-review-lit-review-lit-review-fig-network-synth fig-scap="Example trip distribution using a synthetic population" layout-ncol="2"}
![Trip-based microsimulation](./../images/trip.png){#lit-review-lit-review-lit-review-lit-review-lit-review-lit-review-fig-network-synth-1}

![Activity (tour)-based](./../images/tour.png){#lit-review-lit-review-lit-review-lit-review-lit-review-lit-review-fig-network-synth-2}

Example trip distribution using a synthetic population allows an individual's travel to be tracked.
In the tour-based approach, an attempt is made to make the trips consistent with each other, while the trip-based approach may not be consistent.
:::
...
Because the people within the household will have travel patterns that depend on the patterns of others in the household, a policy affecting one person in the household can affect everyone in the household no matter how directly the policy connects to them [@vovsha_activity-based_2005; @lant2003].
...
Though \acr{ABM}s have many clear theoretical advantages over trip-based models, adoption among agencies has been relatively slow.
...
-   Computational inefficiency and complicated program design: \acr{ABM}s take more time, more computing power, and more money to run.
    ...
    In areas with thousands of \acr{TAZ}s and millions of people, a supercomputer is needed, and it will cost much more than what is spent to run trip-based models.
    If a region can see similar results using a trip-based model, they may decide not to invest in an \acr{ABM}.
...
Although there has been much research on \acr{ABM}s and their theoretical advantages, practical comparisons of the model frameworks have been limited.
...
@ferdous_comparison_2012 compared the trip- and activity-based model frameworks of the Mid-Ohio Regional Planning Commission and found that the \acr{ABM} was slightly more accurate to observed data at the region level, but about the same at the project level.
This is not an overwhelming victory for \acr{ABM}s, and so based on this an agency may reasonably decide that the increase in data, computation, training, and potentially financial requirements outweighs the potential gains of switching to an \acr{ABM}.
...
Additionally, the existing literature comparing the two model types is outdated, and the technology of both model types may have significantly changed in recent years.
