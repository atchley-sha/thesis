...
The aggregate data is also often "disaggregated" and segmented along variables such as household size and vehicle ownership.
...
The following sections will explain these aspects of travel demand modeling and discuss the claimed advantages and disadvantages of each model type.
...
Aggregation at any point precludes that segmentation from use in subsequent model steps as well as in any post-hoc analysis.
...
These types of analysis, which are not possible with aggregate population data (**this is actually possible; a lot of trip-based models segment high/low income**), can be very valuable in transportation planning and policy making.
...
@fig-pipeline-example gives a visualization of an example "information pipeline" for a trip-based model using aggregate data and an \acr{ABM} using a synthetic population.
In the aggregate data model, it is impossible to know which trips are made by e.g. 2-worker, 1-vehicle, low-income households; it only describes which trips are made by households with fewer vehicles than workers.
...
![Trip-based](../images/aggregate.png){#fig-pipeline-example-1}

![Activity-based](../images/synthetic.png){#fig-pipeline-example-2}
...
However, in practice there are few true "activity scheduling" models, and the term "activity-based" is used to refer to both activity scheduling and tour-based models.
...
@fig-network-tbmicro depicts the trip list that could be given for an individual in a trip-based microsimulation model.
...
![Example network assignment using aggregate data. There is little information on who is making which trips, and it is not known how trips are related to each other.](../images/tbm.png){#fig-network-aggregate fig-scap="Example network assignment using aggregate data."}

::: {#fig-network-synth layout-ncol="2" fig-scap="Example network assignment using a synthetic population."}
![Trip-based microsimulation](../images/trip.png){#fig-network-tbmicro}

![Activity (tour)-based](../images/tour.png){#fig-network-abm}

Example network assignment using a synthetic population allows an individual's travel to be tracked.
In the tour-based approach, an attempt is made to make the trips consistent with each other.
:::
