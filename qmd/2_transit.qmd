...
The models differ, however, in where the new commuter rail trips come from.
...
targets::tar_read(combined_tr_mode_split_table) %>%
	...
	width(j = ~cube_diff_pct + asim_diff_pct, 0.1) |>
...
Additionally, there is a nesting structure to the mode choice model in the WFRC model.
...
There are a large number of new commuter rail trips that occur along the entire length of the FrontRunner line, however.
...
@tbl-tr-se-cube shows, for example, the median number of households, people, and jobs weighted by the number of transit trip productions in each TAZ for the WFRC model.
