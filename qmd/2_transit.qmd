...
The highway skims are therefore taken from the baseline scenario in the WFRC model and not updated for this scenario.
...
***Unsurprisingly, both models predict a significant increase in commuter rail trips.*** The models differ, however, in which modes the new commuter rail trips come from.
....
...
For the purposes of this analysis, trips are considered "the same" between scenarios if they share the following: person id, origin and destination zones, time of day[^2_transit-1], and tour and trip purpose.
...
[^2_transit-1]: ActivitySim models time of day as the "departure hour" for each trip.
...
All trips that were "Drive Alone" in the baseline scenario switched to carpool, and there was little mode switching otherwise.
The switching that did occur here can again be largely explained by the randomness in ActivitySim's mode choice models, and again is relatively insignificant.
