# Scenario 2: Improved Transit Service {#2_transit-2_transit-2_transit-2_transit-sec-transit}
...
<!-- Additionally, trains occasionally need to wait for each other in order to pass each other. -->
...
This scenario models these improvements to the FrontRunner.
...
@tbl-tr-mode-split shows the number of trips by purpose and mode for each model, and compares these results between this scenario and the baseline scenario (*n.b.* "local transit" includes both light rail and bus travel).
...
targets::tar_read(combined_tr_mode_split_table) %>%
	...
	align(part = "h", align = "c") |>
	align(j = ~ cube_diff_pct + asim_diff_pct, align = "r") |>
...
#| fig-cap: Trip modes of individuals who switched modes with improved commuter rail service, by tour purpose.
#| fig-scap: Trip modes of individuals who switched modes with improved commuter rail service.
#| fig-width: 6.5
#| fig-height: 8
