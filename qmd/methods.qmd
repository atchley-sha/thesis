# Methodology {#methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-sec-methods}
...
![WFRC model flowchart. Note the feedback loop in the distribution step where preliminary loaded network skims are used to perform subsequent iterations of trip distribution until the distribution converges.](./../images/wfrc_flowchart.png){#methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-fig-wfrc-flowchart fig-scap="WFRC model flowchart."}
...
tribble(
  ...
  flextable() |>
  add_header_row(values = c("Life Cycle", "Presence of persons in household aged:"), colwidths = c(1,3)) |>
  merge_v(part = "header") |>
	align(align = "c", part = "a") |>
	autofit()
```
...
[^methods-1]: https://github.com/byu-transpolab/wfrc_asim_scenario/tree/update-abm-illustration
...
![ActivitySim sub-model flowchart. Long-term decisions are made first, followed by more granular ones.](../images/abmexample.jpg){#methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-fig-asim-flowchart fig-scap="ActivitySim sub-model flowchart."}
...
$$\
where $P(k)$ is the probability of choosing alternative $k$, $V_k$ is the utility of alternative $k$, and $K$ is the set of all alternatives [@mcfadden1974].
...
### PopulationSim {#methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-sec-populationsim}
...
### Validation and Calibration of ActivitySim {#methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-methods-sec-baseline-calibration}
...
Based largely on the model results of @chris-thesis, and partly on the existing (uncalibrated) mode split in ActivitySim, we asserted the following mode shares for ridehail: 0.015% for Home-based Work trips, 0.38% for Home-based Other trips, and 0.4% for Non--home-based trips.

```{r}
#| label: tbl-mode-crosswalk
#| tbl-cap: mode crosswalk
...
targets::tar_read(mc_comparison) |>
	flextable(col_keys = c(
		"purpose", "mode", "asim_trips", "asim_share",
		"blank", "wfrc_trips", "wfrc_share"
	)) |>
	set_header_labels(
		purpose = "Purpose",
		mode = "Mode",
	...
	add_header_row(
		values = c("Purpose", "Mode", "ActivitySim", "", "WFRC Model"),
		colwidths = c(1,1,2,1,2)) |>
	colformat_double(digits = 0) |>
	align(j = ~asim_share + wfrc_share, align = "r") |>
	align(j = 3:7, align = "c", part = "h") |>
	merge_v(1) |>
	merge_v(part = "h") |>
	...
	fix_border_issues() |>
	footnote(
		i = ~mode == "Ridehail", j = ~wfrc_share, ref_symbols = "1",
		value = as_paragraph("Ridehail mode shares were asserted for mode choice calibration,\nbut are not counted here"))
```
...
However, a separate ActivitySim implementation, developed for the Southeast Michigan Council of Governments (SEMCOG) MPO in Michigan, *does* include these submodels, and our ActivitySim implementation takes these submodels directly from the SEMCOG implementation.
...
targets::tar_read(asim_telecommute_model_coeffs) |>
	...
	set_header_labels(
		...
		`4_days_week` = "4 days") |>
	add_header_row(values = c("Description", "Telecommute Frequency Coefficients"), colwidths = c(1, 3)) |>
	merge_v(part = "h") |>
	align(j = 2:4, part = "a", align = "c")
```
