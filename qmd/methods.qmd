...
Because of this, PopulationSim has some amount of randomness in which TAZ it places each household in. As such, for small geographic areas such as TAZs the error distribution between the two models is noisy.
The comparisons in this section are therefore made by aggregating each TAZ at the district level, where each district is defined by \acr{WFRC} and includes several contiguous TAZs.
...
A production-ready synthetic population would match its income distribution more closely to the existing WFRC model, but as mentioned, in this research the focus is on the process, rather than accuracy, for each model.
...
Specifically, an ABM doesn't have a concept of, for example, "home-based work" trips, there are simply trips on a "work" tour, some of which have an origin or destination at home.
