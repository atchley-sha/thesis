# Methodology {#methods-methods-methods-methods-methods-sec-methods}
...
![WFRC model flowchart. Note the feedback loop in the distribution step where preliminary loaded network skims are used to perform subsequent iterations of trip distribution until the distribution converges.](./../images/wfrc_flowchart.png){#methods-methods-methods-methods-methods-fig-wfrc-flowchart fig-scap="WFRC model flowchart."}
...
The HBW and NHBW purposes use only the number of workers per household, and all other trip purposes use the cross-classification of household size with life cycle.
...
The mode choice step uses a choice model to assign a percentage of trips of each purpose to each mode.
...
![ActivitySim sub-model flowchart. Long-term decisions are made first, followed by more granular ones.](../images/abmexample.jpg){#methods-methods-methods-methods-methods-fig-asim-flowchart fig-scap="ActivitySim sub-model flowchart."}
...
### PopulationSim {#methods-methods-methods-methods-methods-sec-populationsim}
...
### Verification and Calibration of ActivitySim {#methods-methods-methods-methods-methods-sec-baseline-calibration}
...
For simplicitly, though, we converted the trips from ActivitySim into purposes that roughly match the WFRC model's purposes.
