...
***also this***.
*Kamryn's notes: I think this does a distribution across household size, income group, and number of workers. Then it does the same thing but categorizes it into high and low incomes. Then it adds the vehicle ownership and outputs 6 dataframes for each household size showing the income (high or low), the number of workers, and the number of vehicles. It also spits out a dataframe with marginal HHsize by life cylce which is then used in trip generation* The model then estimates a joint distribution across these one-dimensional distributions (household size, income group, number of workers, and vehicle ownership).
...
*Kamryn's notes: It looks like the joint distributions aren't actually used in trip generation. It uses the file with marginal HH size by life cycle and the marginal workers file* The joint distributions estimated in the disaggregation step are then used in the trip generation step to estimate the number of trips produced from each TAZ.
