...
Based largely on the model results of @chris-thesis, and partly on the existing (uncalibrated) mode split in ActivitySim, we asserted the following mode shares for ridehail: 0.015% for HBW trips, 0.38% for HBO trips, and 0.4% for NHB trips.
...
targets::tar_read(mc_comparison) |>
	...
	align(j = ~asim_share + wfrc_share, align = "right") |>
	align(j = 3:7, align = "center", part = "h") |>
	merge_v(1) |>
	merge_v(part = "h") |>
...
targets::tar_read(asim_telecommute_model_coeffs) |>
	...
	merge_v(part = "h") |>
	align(j = 2:4, part = "a", align = "center")
```
...
targets::tar_read(comparison_by_telecommute_coeffs) |>
	...
	width(j = 2, 1.3) |>
	width(j = 3:5, 1.2) |>
	align(j = 2:5, align = "center", part = "a")
```
