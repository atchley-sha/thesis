...
It is easy to then model a remote work "rebound effect"^[See @sec-remote-work-considerations] by increasing the utility of a non-mandatory DAP for individuals who work remotely.
...
In this research, we were for example able to make several direct comparisons between ActivitySim and the WFRC model (see Chapters [-@sec-landues]--[-@sec-wfh]).
