...
While it may be possible in a trip-based model to model distance to work as it relates to remote work, it is not clear how best to do this, and may require a separate trip purpose and/or trip distribution model specifically for remote work.
...
Obviously the actual time an agency would require to transition to an ABM depends on many factors such as specific staff experience, but this section is intended to give a very rough approximation of the time and effort needed.
...
Our ActivitySim implementation was no more computationally intensive than the WFRC model, we found relatively easy interoperability between the example Metropolitan Transportation Commission and SEMCOG ActivitySim configurations, and the amount of time and effort required to understand and configure ActivitySim was on the whole rather small.
