...
@sec-methods details the configuration for each model, but a couple key takeaways from this process are discussed here.
...
While ABMs are more complex than trip-based models "under the hood", we found that many of the steps needed to configure ActivitySim would be required in configuring any model (whether trip- or activity-based), such as calibration efforts.

The second is that ActivitySim is relatively easy to customize and extend.
The MTC example configuration our ActivitySim implementation is ultimately based on did not include remote work submodels, but it was simple to copy the remote work models from the SEMCOG example configuration into our implementation.
...
It is simple to add additional variables to ActivitySim's remote work models, on the other hand, as discussed in the previous paragraph.
...
"Non--home-based" trips are not really even a concept in ABMs, as individuals just make trips, some of which begin or end at home.
...
This allows us to see, for example, who "switched" modes after an improvement in transit service (see Figures [-@fig-tr-mode-switching] and [-@fig-tr-atwork-switching]).
Additionally, this allows us to analyze the demographics of individuals using transit, rather than being limited to aggregate data as in a trip-based model.
...
targets::tar_read(cube_time_spent_table) |>
	...
	width(1, 2.4) |>
	width(2, 1.5) |>
	align(j = ~ hours, align = "center", part = "a")
