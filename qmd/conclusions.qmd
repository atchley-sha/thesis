...
With multi-threading set to use 12 threads, and the chunk size set to 102 GB, the baseline ActivitySim scenario ran in about 13 hours.
With multi-threading set to use 16 threads, and the chunk size set to 112 GB, the baseline ActivitySim scenario ran in about XXX hours.
This is still somewhat faster than the WFRC model runs on the same computer, though the difference in runtime is less dramatic than the ActivitySim runs on the BYU supercomputer without chunking.
...
targets::tar_read(cube_time_spent_table) |>
	flextable() |>
	set_header_labels(task = "Task", hours = "Hours spent\n(undergraduate RA)") |>
...
However, these updated skims came from the results of the WFRC model's Transit scenario, and so in some sense the time spent for ActivitySim should possibly include the time spent for the WFRC model (@tbl-time-spent-2 does not include this).

Finally, the tasks were divided between two research assistants almost exclusively in line with the model type.
This means that @tbl-time-spent-1 and @tbl-time-spent-2 are showing the time spent with each model type by a specific individual.
