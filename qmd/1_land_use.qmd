...
This scenario uses the planned residential and commercial development at The Point Utah, the site of the recently demolished Utah State Prison, to model a major increase in land use.
...
This is trivial to implement in the WFRC/MAG model, where all that is needed is a change in the TAZ- level land use data file for the affected TAZs.
The new land use data is the WFRC/MAG predicted 2050 data for these zones, as this is when development of the site is scheduled for completion.
That is, the 2050 socioeconomic data for these five zones (only) was brought into the 2019 scenario.
This added 7,430 new households and 22,200 new jobs to the area.

In ActivitySim, a modified synthetic population with these new households is needed.
Currently, the synthetic population model (PopulationSim) is set up to use Census tract data for many control variables including income.
At the suggestion of Bert Granberg, the BYU team used the Census tract covering the Gateway area in SLC to represent the new development patterns at The Point.
The income distribution, etc. of this area will therefore match that of the Gateway area, though the number of households and zonal SE data will match the WFRC projections for 2050.
The new synthetic population was run for this new development only and added to the base scenario population
