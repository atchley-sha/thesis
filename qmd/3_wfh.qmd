...
While the rebound effect proposes that the *number* of trips may increase on remote work days [see e.g. @he_telecommuting_2015], @mokhtarian_the_1998 find a decrease in vehicle *miles* traveled for both work and discretionary trips on remote work days.
...
targets::tar_read(cube_wfh_by_remote_work_pct_comparison) |>
	...
	flextable(col_keys = c(
		"name", "2019_tc", "2050_tc", "blank", "2019_wfh", "2050_wfh")) |>
	set_header_labels(
		name = "Industry",
		`2019_tc` = "2019",
		`2050_tc` = "2050",
	...
	add_header_row(
		values = c("Industry", "Telecommute %", "", "Home-based Jobs %"),
		colwidths = c(1,2,1,2)
	) |>
	merge_v(part = "h") |>
...
targets::tar_read(comparison_wfh_telecommute_coeffs) |>
	...
	set_header_labels(
		name = "Industry",
		pct = "2050 WFRC Telecommute %"
	) |>
	add_header_row(
		values = c("Industry", "2050 WFRC Telecommute %", "ActivitySim Telecommute Frequency Coefficients"),
		colwidths = c(1, 1, 3)
	) |>
...
targets::tar_read(combined_wfh_mode_split_diff) |>
	...
	flextable(col_keys = c(
		"purpose", "mode", "cube_wfh", "cube_by", "cube_diff_pct",
		"blank", "asim_wfh", "asim_by", "asim_diff_pct"
	)) |>
	set_header_labels(
		purpose = "Purpose",
		mode = "Mode",
		cube_wfh = "Remote Work scenario",
		cube_by = "Baseline scenario",
		cube_diff_pct = "% Change",
		asim_wfh = "Remote Work scenario",
		asim_by = "Baseline scenario",
		asim_diff_pct = "% Change"
	) |>
	add_header_row(
		values = c("Purpose", "Mode", "WFRC Model Trips", "", "ActivitySim Trips"),
...
This figure shows the difference in TLFD for Home-based Work trips between the Remote Work scenario and the baseline scenario.
Note that a point on the positive y-axis indicates an increased density of trips in the Remote Work scenario compared to the baseline, and vice versa.
The WFRC model shows two patterns.
...
Again, though, the WFRC model does not model the interaction between working remotely and commute distance, so this shift in trip length is essentially random, due largely to the distribution of jobs by type.
...
#| fig-cap: Difference in trip length frequency distribution between scenarios for each model.
#| fig-width: 5
#| fig-height: 6
