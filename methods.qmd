...
The synthetic population is tailored via regional targets or controls, which in this case are a combination of the socioeconomic data in the WFRC/MAG model and data from the U.S. Census.
...
In addition to mode split, the trip length frequency distribution (TLFD) ***is good to check***.
This compares the length of trips that are predicted in each model.
@fig-tlfd-comp shows this comparison by mode and purpose.
These distributions match well enough for the purposes of this research, though further calibration would be required to create a production-ready ActivitySim implementation.
...
ActivitySim, on the other hand, has separate "work-from-home" and "telecommute" steps which model both types of work separately, and these can be tuned individually.

The WFRC/MAG model baseline scenario gives telecommute shares as seen in @tbl-wfrc-telecommute.
Only rough calibration is needed for the purposes of this research, and so the overall average of `r targets::tar_read(wfrc_telecommute_pct)` telecommute is taken as a target for ActivitySim.
Similarly, @fig-wfrc-wfh shows the distribution of home-based job percentages in each TAZ, both unweighted and weighted by total TAZ employment and population.
Again, this research simply uses an average of `r targets::tar_read(wfrc_hbj_base_pct)` home-based jobs (work-from-home) to calibrate the ActivitySim work-from-home model.
