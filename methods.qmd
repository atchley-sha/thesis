...
The ActivitySim configuration is based on the San Francisco area, and so coefficients on variables such as travel time and income will be optimized for that area (hence in part why there are so many more transit trips).
...
To effectively compare the models, we propose three scenarios to model and analyze, which are taken to be representative of the types of planning and development that agencies would like to model and analyze:
