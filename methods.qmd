...
For this, the Wasatch Front Regional Council (WFRC) travel demand model is used as a representative trip-based model, and an ActivitySim implementation in the same study area is used as a representative activity-based model (ABM).
...
-   **Land use data.** This includes information about population, employment, and socioeconomic variables such as income, delineated by TAZ. In our case this was provided by WFRC directly, as an output of their land use forecasting model(s).
-   **Travel skims.** These detail trave:l time, cost, etc. between each origin-destination pair of TAZs. The WFRC model uses an iterative process of assigning volumes to the transportation network and recalculating the skims, which are used in the mode and destination choice model steps.
-   **Transportation networks.** These include highway, transit, etc. networks which connect the TAZs to each other. These networks contain information such as link speed and capacity. Though the WFRC model assigns travel volumes to the network, this research does not compare the model's network assignment results. However, the network volumes are still used to calculate the loaded network skims.
-   **Lookup tables.** Many model steps use lookup tables for information such as trip rates. These are taken directly from the WFRC model without modification.
-   **Model constants and coefficients.** Some model steps, including mode choice, require calibration constants and coefficients. These are also taken directly from the WFRC model.
...
The seed sample comes from the ACS PUMS, which contains a sample of actual (anonymized) individuals and households at the PUMA level (PUMAs partition the United States into areas of around 100,000 people each [@census2023]).
The control totals come from two different sources: the Census and the WFRC model.
...
This section discusses three comparisons of interest between the outputs of the two models: mode split, TLFD, and remote work.
...
All three of these scenarios are coded in both the WFRC model and the ActivitySim implementation used.
