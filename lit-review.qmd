...
@fig-network-tbm depicts network assignment in a classic trip-based model where the total number of trips between each zone is given.
With these results, the mode and purpose of each trip is know, but, with aggregate data, there is no way of telling which people made the trips. 
The most that could be done is a segmentation of the households according to vehicle and worker ratios (as shown in @fig-aggregate).
@fig-network-micro and @fig-network-abm, on the other hand, are visual representations of an individual's travel made possible by the use of a synthetic population.   
@fig-network-micro depicts the types of network assignment that could be given in a trip-based microsimulation model. 
Though each individual's trips are known, there is no sense of consistency between trips.
...
The activity-based approach, depicted in @fig-network-abm, adds this consistency by modeling tours, and only generating trips consistent with each tour.
