...
It should be noted that @miller_current_2023 arguest that many current "activity-based" models ought to rather be labelled "tour-based" due to this focus on building tours.
...
@fig-network-example shows an example comparison of the trips assigned to a network in the various model forms.
...
With these results, the mode and purpose of each trip is known, but, with aggregate data, there is no way of telling who made which trips other than the segmentation in the previous steps(see @fig-aggregate). It is
also not possible to construct a coherent daily list of trips for individuals.
...
@fig-network-micro depicts the trip list that could be given for an individual in a trip-based microsimulation model. 
...
The activity-based approach, depicted in @fig-network-abm, attempts to add this consistency by modeling tours, and only generating trips consistent with each tour.
