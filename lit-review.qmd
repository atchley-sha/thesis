...
An \acr{ABM} places the focus on "activities" rather than "trips" as the basic unit of analysis, and predict a sequence of activities for each individual and household, with information such as activity location, start time, and duration, using a high level of temporal and spatial granularity.
...
These matrices list the number of trips between each \acr{TAZ}, and these trips are then assigned to a transportation network (there are various algorithms for doing so, see @ortuzar_modelling_2011).

Activity-based models differ significantly from this approach.
Rather than using aggregate data, \acr{ABM}s use an actual or synthetic population, with individual person and household data [@vovsha_activity-based_2005].
...
If a company is willing to go through the initial work of creating a synthetic population, then they may just use an \acr{ABM} because they believe it will be most accurate.
...
Example "information pipeline" for a trip-based model and an \acr{ABM}.
:::

### Travel Behavior

The other significant difference from trip-based models \acr{ABM}s have (and the main difference from trip-based microsimulation models) is that \acr{ABM}s organize travel into "tours", a sequence of trips that begin and end at the home, rather than just trips.
...
Example network assignment in each model type. Both the trip-based microsimulation model and the \acr{ABM} allow for assignment on an individual level.
:::
...
Though there has been much research on \acr{ABM}s and their theoretical advantages, to the authors' knowledge there is little in the literature about their practicality in the real world.
It is often taken as given that \acr{ABM}s are unilaterally superior to traditional trip-based models due to their better theoretical foundation, but it is not clear if that better foundation always yields better results.
