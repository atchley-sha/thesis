...
@fig-tours-trips-example shows a visual representation of an individual's travel in both a trip-based microsimulation model and an ABM (tour-based). Though each individual's trips are known, in a trip-based model there is no sense of consistency between trips. The activity-based approach adds this consistency by modeling tours, and only generating trips consistent with each tour.

```{r}
#| label: fig-tours-trips-example
#| fig-cap: "Example travel pattern for an individual in both a trip-based microsimulation model and an ABM."
#| fig-subcap: 
